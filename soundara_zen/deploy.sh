...
server {
    ...
    location ~ ^/(health|library|user_library|user_playlists|user_subscriptions|process|process_audio|track_event|create_checkout_session|create_subscription_session|submit_survey|play|webhook|admin|api/referral|api/interest|api/envision|api/config|demo) {
